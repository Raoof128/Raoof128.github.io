...
// Widget/QRShieldWidget.swift
// QR-SHIELD Lock Screen Widget - iOS 17+
//
...
// 2. Name it "QRShieldWidget"
// 3. Replace the generated code with this file
// 4. Add the URL scheme "qrshield://" to the main app's Info.plist
...
struct QRShieldProvider: TimelineProvider {
    func placeholder(in context: Context) -> QRShieldEntry {
        QRShieldEntry(date: Date())
    }

    func getSnapshot(in context: Context, completion: @escaping (QRShieldEntry) -> ()) {
        let entry = QRShieldEntry(date: Date())
        completion(entry)
    }
...
struct QRShieldEntry: TimelineEntry {
    let date: Date
}

// MARK: - Widget Views

struct QRShieldWidgetEntryView: View {
    var entry: QRShieldProvider.Entry
...
struct AccessoryCircularView: View {
    var body: some View {
        ...
        .widgetURL(URL(string: "qrshield://scan"))
    }
}
...
@main
struct QRShieldWidget: Widget {
    let kind: String = "QRShieldWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: QRShieldProvider()) { entry in
            QRShieldWidgetEntryView(entry: entry)
                .containerBackground(.fill.tertiary, for: .widget)
        }
        .configurationDisplayName("QR-SHIELD")
