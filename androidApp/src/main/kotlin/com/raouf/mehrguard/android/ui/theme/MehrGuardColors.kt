...
package com.raouf.mehrguard.android.ui.theme
 ...
 * Usage: MehrGuardThemeColors.background (auto-selects based on theme)
 */
object MehrGuardThemeColors {
    val background: Color
        @Composable @ReadOnlyComposable
        get() = if (isSystemInDarkTheme()) MehrGuardColors.BackgroundDark else MehrGuardColors.BackgroundLight
    
    val surface: Color
        @Composable @ReadOnlyComposable
        get() = if (isSystemInDarkTheme()) MehrGuardColors.SurfaceDark else MehrGuardColors.SurfaceLight
    
    val surfaceVariant: Color
        @Composable @ReadOnlyComposable
        get() = if (isSystemInDarkTheme()) MehrGuardColors.SurfaceDarkAlt else MehrGuardColors.Slate100
    
    val textPrimary: Color
        @Composable @ReadOnlyComposable
        get() = if (isSystemInDarkTheme()) MehrGuardColors.TextPrimaryDark else MehrGuardColors.TextPrimaryLight
    
    val textSecondary: Color
        @Composable @ReadOnlyComposable
        get() = if (isSystemInDarkTheme()) MehrGuardColors.TextSecondaryDark else MehrGuardColors.TextSecondaryLight
    
    val border: Color
        @Composable @ReadOnlyComposable
        get() = if (isSystemInDarkTheme()) MehrGuardColors.Slate700 else MehrGuardColors.Slate200
    
    val borderVariant: Color
        @Composable @ReadOnlyComposable
        get() = if (isSystemInDarkTheme()) MehrGuardColors.Slate800 else MehrGuardColors.Slate100
}

/**
 * Spacing values matching Tailwind's spacing scale
 */
object MehrGuardSpacing {
